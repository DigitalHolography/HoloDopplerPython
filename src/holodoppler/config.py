...
class ProcessingParameters:
    ...
    shack_hartmann_ny_subap: int = field(default=5, metadata={"label": "Shack-Hartmann Y subapertures"})
    shack_hartmann_nx_subap: int = field(default=5, metadata={"label": "Shack-Hartmann X subapertures"})
    shack_hartmann_zernike_fit: bool = field(default=True, metadata={"label": "Zernike fit"})
    shack_hartmann_zernike_fit_modes: list[int] = field(
        default_factory=lambda: [4, 5, 6],
        metadata={"label": "Zernike fit modes"},
    )
    shack_hartmann_southwell_phase_integration: bool = field(
        default=False,
        metadata={"label": "Southwell phase integration"},
    )
    ...
    def to_dict(self) -> dict[str, Any]:
        self.validate()
        parameter_dict = asdict(self)
        parameter_dict["shack_hartmann_southwell_phase_integration "] = parameter_dict.pop(
    ...
    def validate(self) -> None:
        ...
        if self.shack_hartmann_ny_subap <= 0:
            raise ValueError("shack_hartmann_ny_subap must be greater than 0.")
        if self.shack_hartmann_nx_subap <= 0:
            raise ValueError("shack_hartmann_nx_subap must be greater than 0.")
        if not self.shack_hartmann_zernike_fit_modes:
            raise ValueError("shack_hartmann_zernike_fit_modes cannot be empty.")
        if any(mode < 0 for mode in self.shack_hartmann_zernike_fit_modes):
